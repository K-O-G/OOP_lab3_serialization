...
namespace CustomSerialization
{
    class workCustom
    {
        ...
        {
            CollectionBinary collection = new CollectionBinary();
            collection.Add(new ProductCustom(8, "Chocolate", "Roshen", 20, 2000));
            collection.Add(new ProductCustom(1, "Noodles", "FineFood", 25, 3100));
            collection.Add(new ProductCustom(10, "Sausages", "Yatran", 50, 1000));
            collection.Add(new ProductCustom(5, "Cheese", "Como", 50, 84000));
            collection.Add(new ProductCustom(3, "Wine", "Shabo", 80, 500));

            Console.WriteLine("Unsorted elements:");
            foreach (ProductCustom serial_unsort in collection)
            {
            ...
            foreach (ProductCustom serial_sorted in collection)
            {
                //code,name,serial,producer,price,amount
        ...
        {
            CollectionBinary serial_read = new CollectionBinary();
            try
            {
                using (Stream stream = File.Open("data.bin", FileMode.Open))
                {
                    BinaryFormatter bin = new BinaryFormatter();
                    serial_read = (CollectionBinary)bin.Deserialize(stream);
                }
            ...
            foreach (ProductCustom serial in serial_read)
            {
